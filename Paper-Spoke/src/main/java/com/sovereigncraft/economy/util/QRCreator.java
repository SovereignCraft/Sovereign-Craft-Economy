...
public class QRCreator {
    ...
    public QRCreator(PaperSpoke plugin) {
        this.plugin = plugin;
    }

    @SneakyThrows
    public void generate(String string, Player player, String data) {

        ItemStack map = MapCreator.generateMap(plugin, string, player, data, false);

        player.getInventory().setItemInMainHand(map);
        String id = String.valueOf(((MapMeta) map.getItemMeta()).getMapId());

        File mapsData = new File(plugin.getDataFolder(), "data.yml");
